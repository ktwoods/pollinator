  ...
  // Attempt to submit log, and print success/fail alert
	if (isset($_POST['latin'])) // Submitting data
	{
		...
		if ($stmt->execute())
		{
			...
			else echo "<div class='alert alert-warning alert-dismissable text-center'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Error: unable to add entry for <em>$latin</em>, $date to the database.</div>";
		}
	}
	?>
	...
	<form action="new_log.php" method="post">
        ...
        <!-- Stage ('adult' selected by default) -->
